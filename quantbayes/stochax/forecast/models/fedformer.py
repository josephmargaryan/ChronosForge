...
def decompose(x: jnp.ndarray, kernel_size: int) -> tuple[jnp.ndarray, jnp.ndarray]:
    ...
    trend = jax.vmap(lambda v: jnp.convolve(jnp.reshape(v, (-1,)), kernel, mode="same"))(x.T).T
    seasonal = x - trend
...
class TransformerBlock(eqx.Module):
    ...
    def __init__(self, embed_dim: int, num_heads: int,
                 mlp_ratio: float = 4.0, dropout_p: float = 0.1, *, key):
    ...
    def __call__(self, x, *, key=None):
        ...
        mlp_out = jax.vmap(self.mlp)(y, key=None if key_mlp is None else jr.split(key_mlp, x.shape[0]))
        x = x + mlp_out
        return x
...
class Fedformer(eqx.Module):
    ...
    def __init__(self, input_dim: int, embed_dim: int, num_heads: int,
                 mlp_ratio: float = 4.0, dropout_p: float = 0.1,
                 kernel_size: int = 3, *, key):
        ...
        self.trend_net = MLP(in_features=embed_dim, hidden_features=int(embed_dim * mlp_ratio),
                             dropout_p=dropout_p, key=key_trend)
    ...
    def __call__(self, x: jnp.ndarray, *, key=None) -> jnp.ndarray:
        ...
        seasonal, trend = decompose(x_proj, self.kernel_size)  # both [seq_len, embed_dim]
        if key is not None:
            key_seasonal, key_trend = jr.split(key)
        else:
            key_seasonal = key_trend = None
        seasonal_out = self.seasonal_block(seasonal, key=key_seasonal)  # [seq_len, embed_dim]
        trend_out = self.trend_net(trend[-1], key=key_trend)             # [embed_dim]
        last_seasonal = seasonal_out[-1]                                 # [embed_dim]
        final_feat = jnp.concatenate([last_seasonal, trend_out], axis=-1) # [2*embed_dim]
...
class FedformerForecast(eqx.Module):
    ...
    def __init__(self, input_dim: int, embed_dim: int, num_heads: int,
                 mlp_ratio: float = 4.0, dropout_p: float = 0.1,
                 kernel_size: int = 3, *, key):
        self.model = Fedformer(input_dim, embed_dim, num_heads, mlp_ratio, dropout_p,
                                kernel_size, key=key)
