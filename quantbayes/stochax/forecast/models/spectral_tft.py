...
class SpectralMultiheadAttention(eqx.Module):
    ...
    def __init__(self, embed_dim: int, seq_len: int, num_heads: int, dropout_rate: float, *, key: PRNGKeyArray):
...
class SpectralTemporalFusionTransformer(eqx.Module):
    ...
    def __init__(self, input_size: int, hidden_size: int, num_heads: int, seq_len: int, *, key: PRNGKeyArray):
        ...
        self.spectral_attn = SpectralMultiheadAttention(embed_dim=hidden_size, seq_len=seq_len, num_heads=num_heads, dropout_rate=0.1, key=k2)
    ...
    def __call__(self, x: jnp.ndarray, *, key: PRNGKeyArray) -> jnp.ndarray:
        ...
        attn_out = self.spectral_attn(hs, training=False, key=key)  # shape: (seq_len, hidden_size)
...
if __name__ == "__main__":
    ...
    model = SpectralTemporalFusionTransformer(input_size=D, hidden_size=D, num_heads=4, seq_len=seq_len, key=model_key)
