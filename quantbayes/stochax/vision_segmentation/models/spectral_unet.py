...
def spectral_conv2d_jvp(primals, tangents):
    ...
    dX_fft = jnp.fft.fftn(dx, axes=(0,1)) if dx is not None else 0.
    term1 = dX_fft * fft_mask
    term2 = X_fft * dmask if dmask is not None else 0.
...
class SpectralConvBlock(eqx.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, spatial_shape: Tuple[int,int], *, key):
        k1, k2 = jr.split(key)
        # First, use a standard conv to map to out_channels.
        self.conv = eqx.nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1, key=k1)
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        if self.use_spectral:
            ...
            return jax.vmap(lambda xi, mask: spectral_conv2d(xi, mask))(x, self.fft_mask)
        else:
            return x
