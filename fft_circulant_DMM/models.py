...
class GuideRNN(nn.Module):
    def __init__(self, x_dim, z_dim, rnn_dim=64, num_layers=1):
        ...
        self.x_dim = x_dim
        self.z_dim = z_dim
        self.rnn = nn.GRU(input_size=x_dim, hidden_size=rnn_dim,
    ...
    def model(self, padded_x, masks=None):
        ...
        with pyro.plate("sequences", batch_size):
            z_prev = pyro.sample("z_0",
                                 dist.Normal(self.z_0_loc, z_0_scale).to_event(1))

            for t in pyro.markov(range(T)):
                z_loc, z_scale = self.trans(z_prev)
                z_t = pyro.sample(f"z_{t+1}",
                                  dist.Normal(z_loc, z_scale).to_event(1))

                x_loc, x_scale = self.emission(z_t)
                pyro.sample(f"x_{t+1}",
    ...
    def guide(self, padded_x, masks=None):
        ...
        with pyro.plate("sequences", batch_size):
            ...
            for t in pyro.markov(range(T)):
                ...
                q_scale = scale_seq[:, t, :]
                z_t = pyro.sample(f"z_{t+1}",
                                  dist.Normal(q_loc, q_scale).to_event(1))
                z_prev = z_t
