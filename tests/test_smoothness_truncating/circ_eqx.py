...
class SmoothTruncEquinoxCirculant(eqx.Module):
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        if (n % 2 == 0) and (self.k_half > 1):
            ...
            fft_full = jnp.concatenate([
                half_complex,
                jnp.conjugate(half_complex[1:])[::-1]
            ])
...
def plot_prior_posterior_frequency(
    ...
    axes[0].errorbar(freqs, post_mean_r, yerr=post_std_r, fmt='b.-', capsize=3, label="Posterior (real)")
    ...
    axes[1].errorbar(freqs, post_mean_i, yerr=post_std_i, fmt='g.-', capsize=3, label="Posterior (imag)")
